...
public class SpinGUI2 extends JFrame implements ActionListener{
    ...
    private int max = 20;
    private int rand, i, count=0, c=0;
    ...
    public void displayRandomImages(int index1, int index2, int index3){
		//getScaledInstance(int width,int height,) method in package Image
			   ...
			   Thread thread = new Thread(new Runnable(){
					public void run(){
						tm = new Timer(20, new ActionListener(){
							public void actionPerformed(ActionEvent ae){
								randomNumbers();
								displayRandomImages(index1,index2,index3);
								if(c==images.length){
									c = 0;
									tm.stop();
									if(isWon() == true){
